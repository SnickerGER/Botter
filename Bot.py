#Developed by github.com/SnickerGER
...
{self.console_msg("4")} Share Bot {Fore.RED}{self.status["shares"]}
    ...
    def update_title(self, bot, remaining, rl = False):
        if clear == "cls":
            os.system(clear)
            ctypes.windll.kernel32.SetConsoleTitleW(f"TikTok AIO | Sent: {self.sent} | Cooldown: {remaining}s | Developed by @SnickerGER on Github")
            print(ascii_text)
